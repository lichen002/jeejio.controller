...
public class ApplyTest {
    ...
    public void testApplyInstall() throws IOException, InterruptedException {
        ...
        getDevicesInstallApply.setSessionId(sessionId);
        getDevicesInstallApply.setToken(token);
        ...
        for (int i=4;i<appList.size();i++){
            ...
            applyUnInstall.setSessionId(sessionId);
            applyUnInstall.setToken(token);
        ...
        JSONObject appResultMarket = utils.getApplyList(TEST_MACHINE_CODE, "10008", userId, userCode,token,sessionId);
        log.info("appResultMarket is {}",appResultMarket);
        JSONArray resultListMarket =(JSONArray)appResultMarket.get("resultList");

        for (int i=0;i<resultListMarket.size()-10;i++){
            sleep(500);
            ...
            applyInstall.setSessionId(sessionId);
            applyInstall.setSource("2");
            applyInstall.setToken(token);
...
//        Assert.assertEquals(5,appList2.size());
    ...
    public void testIsExistNotInstall() throws IOException {
        JSONObject appResult = utils.getApplyList(TEST_MACHINE_CODE, "10008", userId, userCode,token,sessionId);
        log.info("appResult is {}",appResult);
...
//    }
    ...
    public void testApplyOpen() throws IOException, InterruptedException {
        ...
        for (int i=4;i<appList.size();i++){
            ...
            applyOpen.setSessionId(sessionId);
            applyOpen.setToken(token);
    ...
    public void testApplyClose() throws IOException, InterruptedException {
        ...
        for (int i = 4; i < appList.size(); i++) {
            ...
            applyClose.setSessionId(sessionId);
            applyClose.setToken(token);
